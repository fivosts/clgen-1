...
try:
  ...
  CLANG               = check_exists(os.environ['CLANG'],        
                        "phd/third_party/llvm/clang"
                        )
  CLANG_FORMAT        = check_exists(os.environ['CLANG_FORMAT'],
                        "phd/third_party/llvm/clang-format"
                        )
  ...
  CLGEN_REWRITER      = check_exists(os.environ['CLANG_REWRITER'],
                        "phd/deeplearning/clgen/preprocessors/clang_rewriter"
                        )
  LIBCLC              = check_exists(os.environ['LIBCLC'],
                        "phd/third_party/libclc/generic/include"
                        )
  ...
  DASHBOARD_STATIC    = check_exists(os.environ['DASHBOARD_STATIC'],
                        "phd/deeplearning/clgen/dashboard/static"
                        )
  DATA_CL_INCLUDE     = check_exists(os.environ['DATA_CL_INCLUDE'],
                        "phd/deeplearning/clgen/data/include/"
                        )
